...
为 `references/platform-playbooks/*.md` 归纳"Atoms builder（应用创建者）可复用的 built app 社媒高转化模式"章节收集有效样本。
...
| **Instagram** | ❌ 已剥离(2026-07-02 实测) | Category B(行业博客) + Category D(人工补齐) | IG OG 抓取路线已废弃并从当前 repo 删除;只保留 Category B/D 离线研究路径 |
...
Step 1  搜索博客案例     →  WebSearch (Category B/C queries in search_queries.md)
                            → references/research-data/instagram/industry_urls.txt(博客文章 URL,一行一个)

Step 2  提取结构化案例   →  逐个 WebFetch,LLM 按 case_study_schema.json 提取
                            → references/research-data/instagram/case_studies.json(结构化案例数组)

Step 3  人工补齐         →  用户提供 blogs 未覆盖的高质量样本
                            → references/research-data/instagram/manual_supplements.md(markdown 模板逐条填)
...
| 文件 | 内容 | 来源 |
|---|---|---|
| `references/research-data/instagram/industry_urls.txt` | 博客/官方文档 URL 列表 | Category B/C WebSearch |
| `references/research-data/instagram/case_studies.json` | 结构化案例(brand / caption / hashtag / engagement / insight) | WebFetch + LLM 提取 |
| `references/research-data/instagram/manual_supplements.md` | 用户提供的补齐样本 | 人工填 |
| `references/research-data/schemas/case_study_schema.json` | 提取 schema | 见文件 |
...
- IG OG scraper 及旧空样本产物已删除。当前 pipeline 只保留行业博客案例 + 人工补齐;若未来恢复合规公开接口,应按 v3 data access profile 重新设计 adapter。
...
Step 1  收集 YT 视频 URL      →  按 scripts/youtube_search_queries.md § A 里的 query
                                ...
                                → references/research-data/youtube/industry_urls.txt(§ A 分类下)

Step 2  批量抓元数据          →  运行 fetch_youtube_metadata.py(oembed 或 api 模式)
                                → references/research-data/youtube/video_samples.json(结构化元数据)

Step 3  搜行业博客深度案例    →  WebSearch (Category B queries in youtube_search_queries.md)
                                → references/research-data/youtube/industry_urls.txt(§ B 分类下)+ 逐个 WebFetch
                                → references/research-data/youtube/case_studies.json(带 CTR/AVD/AVP 深度数据)

Step 4  官方文档核对          →  Category C 已知 URL 直接 WebFetch(见 youtube_industry_urls.txt § C)
                                → 用于 §3 算法机制 / §7 商业化 / policy 引用

Step 5  人工补齐              →  用户按 references/research-data/youtube/manual_supplements.md 模板填
                                → 补失败/中位样本、非英语市场、Shorts 3min 新形式、Atoms builder（应用创建者）的 built app 自建频道
...
| 文件 | 内容 | 来源 |
|---|---|---|
| `references/research-data/youtube/industry_urls.txt` | Category A/B/C 分组 URL(视频 + 博客 + 官方) | 手动种子 + WebSearch 补齐 |
| `references/research-data/youtube/video_samples.json` | 每支视频的结构化元数据(oEmbed 或 API v3 字段) | `fetch_youtube_metadata.py` 输出 |
| `references/research-data/youtube/case_studies.json` | 带深度洞察的博客案例(含 CTR/AVD/AVP) | WebFetch + LLM 按 schema 提取 |
| `references/research-data/schemas/youtube_case_study_schema.json` | 提取 schema(与 IG 版本对齐,差异见 `$fields_vs_ig_schema_diff`) | 见文件 |
| `references/research-data/youtube/manual_supplements.md` | 用户提供的补齐样本 | 人工填 |
| `references/research-data/youtube/errors.txt` | fetcher 失败样本(私有/删除/id 无效等) | fetcher 输出 |
...
python3 scripts/fetch_youtube_metadata.py \
    --input references/research-data/youtube/industry_urls.txt \
...
**脚本特性**:
- video_id 去重(重跑不会重复抓)
- 失败样本记录到 `references/research-data/youtube/errors.txt`,不阻塞整体流程
...
- **Category D(人工补齐)**:**6-12 个**——补失败/中位样本、非英语、Shorts 3min 新形式、Atoms builder（应用创建者）的 built app 自建频道
- **合计 45-80**
...
Step 1  搜索博客案例       →  WebSearch (Category B/C queries in tiktok_search_queries.md)
                              → references/research-data/tiktok/industry_urls.txt(按 B1/B2/B3/B4/B5/C 分组)

Step 2  提取结构化案例     →  逐个 WebFetch,LLM 按 case_study_schema.json + TikTok 独有字段提取
                              → references/research-data/tiktok/case_studies.json

Step 3  人工补齐(重度)   →  用户提供 blogs 未覆盖的样本(尤其 Creator 类 solo builder ≥10 条)
                              → references/research-data/tiktok/manual_supplements.md

Step 4  Creative Center 快照 → WebFetch 4 类子页(hashtag / sound / top ads / keyword insights)
                              → .cache/social_intel/manual/tiktok/{region}/{language}/{YYYY-Www}/creative-center.json(2-4 周刷新)
...
| 文件 | 内容 | 来源 | 刷新节奏 |
|---|---|---|---|
| `references/research-data/tiktok/industry_urls.txt` | 博客 + 官方 URL 池,按 B1/B2/B3/B4/B5/C/E 分组 | Category B/C WebSearch | 6 个月 |
| `references/research-data/tiktok/case_studies.json` | 结构化案例(含 video_length / sound_type / hook_pattern / loop_designed / fyp_penetration) | WebFetch + LLM 提取 | 12 个月 |
| `references/research-data/tiktok/manual_supplements.md` | 用户提供样本(重点 Creator / vibe coding SMB) | 人工填 | 按增量 |
| `.cache/social_intel/manual/tiktok/{region}/{language}/{YYYY-Www}/creative-center.json` | Creative Center 4 类趋势数据(hashtags / sounds / top ads patterns / keyword insights) | WebFetch(Category E) | **2-4 周** |
...
- Atoms builder（应用创建者）的 built app 早期社媒账号 90%+ 在 0-1K subs / 0-1K followers 区间——归纳时优先取小账号可复制的样本,大账号(>100K)结论标注"仅供品牌方向参考"
